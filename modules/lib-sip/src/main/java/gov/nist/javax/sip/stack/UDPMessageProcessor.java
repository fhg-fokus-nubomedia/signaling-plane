...
public class UDPMessageProcessor extends MessageProcessor {
    ...
    protected UDPMessageProcessor(InetAddress ipAddress,
        ...
        try {
            if (ipAddress instanceof Inet4Address)
                this.sock = sipStack.getNetworkLayer().createDatagramSocket(InternetProtocolFamily.IPv4);
            else
                this.sock = sipStack.getNetworkLayer().createDatagramSocket(InternetProtocolFamily.IPv6);
    ...
    public void run() {
        ...
        final UDPMessageProcessor processor = this;
        String ip = this.getIpAddress().toString().replace('/',' ').trim();


        sock.listen(ip, this.port, new Handler<AsyncResult<DatagramSocket>>() {
            public void handle(AsyncResult<DatagramSocket> datagramSocketAsyncResult) {
                if (datagramSocketAsyncResult.succeeded()) {
                    sock.dataHandler(new Handler<DatagramPacket>() {
                        @Override
                        public void handle(DatagramPacket datagramPacket) {
                            UDPMessageChannel pkg =  new UDPMessageChannel(sipStack, processor, datagramPacket);
                            try {
                                pkg.processIncomingDataPacket(datagramPacket);
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
                        }
                    ...
                    System.out.println("Problem : " + datagramSocketAsyncResult.cause());
                }
            }
        });
