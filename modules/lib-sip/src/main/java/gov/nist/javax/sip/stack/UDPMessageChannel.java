...
public class UDPMessageChannel extends MessageChannel implements
     ...
     *
     * @param sipMessage the SipMessage
     */
    public void processMessage(SIPMessage sipMessage) {
        ...
        if (sipMessage instanceof SIPRequest) {
            ...
            if (sipServerRequest == null) {
                logger.log(LogWriter.TRACE_WARN, "Null request interface returned -- dropping request");
                return;
            }
            logger.log(LogWriter.TRACE_DEBUG,  "About to process " + sipRequest.getFirstLine() + "/"
                    + sipServerRequest);
            try {
            ...
            logger.log(LogWriter.TRACE_DEBUG, "Done processing " + sipRequest.getFirstLine() + "/"
                    + sipServerRequest);
            // So far so good -- we will commit this message if
            // all processing is OK.
            ...
            try {
                ...
                logger.log(LogWriter.TRACE_ERROR,  "Dropping Badly formatted response message >>> "
                        + sipResponse);
                return;
            }
            ...
            if (sipServerResponse != null) {
                try {
                    if (sipServerResponse instanceof SIPClientTransaction
                        ...
                        logger.log(LogWriter.TRACE_ERROR, "Dropping response message with invalid tag >>> "
                                + sipResponse);
                        return;
                    }
                ...
                logger.log(LogWriter.TRACE_DEBUG, "null sipServerResponse!");
            }
    ...
    public void handleException(ParseException ex, SIPMessage sipMessage,
            ...
            throws ParseException {
        logger.log(LogWriter.TRACE_EXCEPTION, ex);
        ...
        if ((hdrClass != null)
            ...
            logger.log(LogWriter.TRACE_ERROR,"BAD MESSAGE! :"  + message);
            throw ex;
        } else {
    ...
    protected void sendMessage(byte[] msg, InetAddress peerAddress,
        ...
        try {
             sipStack.ioHandler.sendBytes(null, peerAddress, peerPort, Transport.UDP, msg, false);
        } catch (IOException ex) {
            throw ex;
        } catch (Exception ex) {
    ...
    protected void sendMessage(byte[] msg, InetAddress peerAddress,
        ...
        // Via is not included in the request so silently drop the reply.
        if (peerPort == -1) {
            logger.log(LogWriter.TRACE_DEBUG, getClass().getName() + ":sendMessage: Dropping reply!");
            throw new IOException("Receiver port not set ");
        } else {
            logger.log(LogWriter.TRACE_DEBUG,  ":sendMessage " + peerAddress.getHostAddress() + "/"
                    + peerPort + "\n" + " messageSize = "
                    + msg.length);
        }
        if (peerProtocol.compareToIgnoreCase("UDP") == 0) {
            try {
                logger.log(LogWriter.TRACE_ERROR, "sendMessage " + peerAddress.getHostAddress() + "/"
                        + peerPort + "\n" + new String(msg));
                sipStack.ioHandler.sendBytes(null, peerAddress, peerPort, Transport.UDP, msg, false);
            } catch (IOException ex) {
                throw ex;
            ...
            if (sipStack.ioHandler.sendBytes(
                    this.messageProcessor.getIpAddress(), peerAddress,
                    peerPort, Transport.TCP, msg, false) == null) {
                logger.log(LogWriter.TRACE_ERROR, "sendMessage");
            }
        }
    }
