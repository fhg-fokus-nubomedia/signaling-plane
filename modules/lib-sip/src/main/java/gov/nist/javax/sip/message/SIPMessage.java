...
public abstract class SIPMessage extends MessageObject implements javax.sip.message.Message,
    ...
    //Table of Record Routes
    protected LinkedList<SIPHeader> recordRouteTable;
...
//        StringBuilder encoding = new StringBuilder();
    ...
    public SIPMessage() {
        ...
        this.recordRouteTable = new LinkedList<SIPHeader>();
        try {
    ...
    public void attachHeader(SIPHeader header, boolean replaceFlag, boolean top)
        ...
        if (header instanceof RecordRouteList) {
        ...
        if (((!headerTable.containsKey(headerNameLowerCase)) && !(header instanceof RecordRouteList))
        		|| headerNameLowerCase.compareToIgnoreCase("route") == 0 
        		|| header instanceof RecordRouteHeader) {
        ...
        if (header instanceof RecordRouteList) {
        	Iterator<RecordRoute> element = ((RecordRouteList) header).iterator();
        	Iterator<SIPHeader> rr = recordRouteTable.iterator();
        	//dup check
        	int i = 0;
        	SIPHeader rrh = null;
        	SIPHeader rrInHeader = null;
        	while (rr.hasNext()) {
        		rrh = rr.next();
        		element = ((RecordRouteList) header).iterator();
        		while (element.hasNext()) {
        			rrInHeader = element.next();
        			if (rrh.getHeaderValue().hashCode() == rrInHeader.getHeaderValue().hashCode()) {
        				System.out.println("RR remove element with index :" + i + "Have :" +rrh.getHeaderValue() + "Get :" +rrInHeader.getHeaderValue());
        				this.recordRouteTable.remove(i);
        			}
        		}
        		i++;
        	}
        	element = ((RecordRouteList) header).iterator();
        	while (element.hasNext()) {
        		this.recordRouteTable.add((RecordRoute) element.next());
        	}
        }
    ...
    /**
     * because the programmer of this stack was to ... to make it right 
     * @param header
     */
    public void attachDefaultRoute(Header header) {
    	SIPHeader sh = (SIPHeader) header;
    	if (sh instanceof RouteList) {
    		  String headerNameLowerCase = SIPHeaderNamesCache.toLowerCase(sh.getName());
    		  headerTable.put(headerNameLowerCase, sh);
    	}
    }
    
    public void removeRouteHeader(SipUri uri) {
    	SipUri headerUri;
    	RouteHeader route;
        ...
        while (li.hasNext()) {
            SIPHeader sipHeader = (SIPHeader) li.next();
            if (sipHeader instanceof RouteHeader) {
            	route = (RouteHeader) sipHeader;
            	headerUri = (SipUri) route.getAddress().getURI();
           	 	headerUri.removeParameters();
            	if (uri.equals(headerUri)) {
            		this.headers.remove(sipHeader);
            	}
            }
        }
    }
...
//        }
    ...
    /**
     * Return an iterator for the list of recordRoutes in this message.
     *
     * @return an Iterator for the headers of this message.
     */
    public ListIterator<SIPHeader> getRecordRouts() {
    	return this.recordRouteTable.listIterator();
    }
    ...
    public ListIterator<SIPHeader> getHeaders(String headerName) {
        ...
        if (headerName.toLowerCase().equals("record-route")) { //used to get the RecordRoutes
        	return (ListIterator<SIPHeader>)this.getRecordRouts();
        } else {
    ...
    public SIPHeader getSIPHeaderListLowerCase(String lowerCaseHeaderName) {
    	if (lowerCaseHeaderName.equals("route")) {
