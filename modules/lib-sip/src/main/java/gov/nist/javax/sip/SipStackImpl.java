...
public class SipStackImpl extends SIPTransactionStack implements
    ...
    private SipStackImpl(Vertx vertx) {
		super(vertx);
	 ...
	 */
	public SipStackImpl(Properties configurationProperties, Vertx vertx)
			throws PeerUnavailableException {
		this(vertx);
		...
		if (configurationProperties.containsKey(NETWORK_LAYER_KEY)) {
			...
			try {
				...
				networkLayer = (VertxNetworkLayer) c.newInstance(new Object[0]);
	...
	public synchronized ListeningPoint createListeningPoint(String address,
        ...
        System.out.println("ListeningPoint : " + transport);
		if (logger.isLoggingEnabled(LogLevels.TRACE_DEBUG))
			logger.logDebug(
		...
		if (lip != null) {
			...
			try {
                ...
                System.out.println("ListeningPoint K");
				if (logger.isLoggingEnabled())
					logger.logError(
