...
public class SipProviderImpl implements javax.sip.SipProvider, gov.nist.javax.sip.SipProviderExt,
	...
	public void handleEvent(EventObject sipEvent, SIPTransaction transaction) {
		if (logger.isLoggingEnabled(LogLevels.TRACE_DEBUG)) {
			logger.logDebug("handleEvent " + sipEvent + "currentTransaction = " + transaction + "this.sipListener = "
					+ this.getSipListener() + "sipEvent.source = " + sipEvent.getSource());
	...
	protected SIPClientTransaction createClientTransaction(Request request, Hop hop)
		...
		if (sipRequest.getMethod().equalsIgnoreCase(Request.CANCEL)) {
			SIPClientTransaction ct = (SIPClientTransaction) sipStack.findCancelTransaction((SIPRequest) sipRequest,
					false);
			if (ct != null) {
		...
		if (logger.isLoggingEnabled(LogLevels.TRACE_DEBUG))
			logger.logDebug("could not find existing transaction for " + sipRequest.getFirstLine()
					+ " creating a new one ");
	...
	public ServerTransaction getNewServerTransaction(Request request) throws TransactionAlreadyExistsException,
		...
		try {
			if (SIPTransactionStack.isDialogCreated(sipRequest.getMethod())) {
				...
				if (isAutomaticDialogSupportEnabled()) {
					/*
					 * Under automatic dialog support, dialog is tied into a
					 * transaction. You cannot create a server tx except for
					 * dialog creating transactions. After that, all subsequent
					 * transactions are created for you by the stack.
					 */
					...
					// If there is a dialog already assigned then just update
					// the
					// dialog state.
					String dialogId = sipRequest.getDialogId(true);
					SIPDialog dialog = sipStack.getDialog(dialogId);
	...
	public void sendRequest(Request request) throws SipException {
		...
		try {
			...
			if (this.listeningPoints.containsKey(hop.getTransport().toUpperCase())) {
				messageChannel = sipStack.createRawMessageChannel(this.getListeningPoint(hop.getTransport())
						.getIPAddress(), this.getListeningPoint(hop.getTransport()).getPort(), hop);
			}
	 ...
	 * 
	 * @see
	 * gov.nist.javax.sip.SipProviderExt#setDialogErrorsAutomaticallyHandled()
	 */
